...
class FlutterCallkitIncomingPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    companion object {
        ...
        /**
         * Ensures native ringtone/notification managers exist before a broadcast
         * receiver handles an incoming call (FCM background / cold start).
         */
        fun ensureManagers(context: Context) {
            val appContext = context.applicationContext
            if (!::instance.isInitialized) {
                instance = FlutterCallkitIncomingPlugin()
            }
            if (instance.callkitSoundPlayerManager == null) {
                instance.callkitSoundPlayerManager = CallkitSoundPlayerManager(appContext)
            }
            if (instance.callkitNotificationManager == null) {
                instance.callkitNotificationManager = CallkitNotificationManager(
                    appContext,
                    instance.callkitSoundPlayerManager,
                )
            }
