...
class CallkitIncomingBroadcastReceiver : BroadcastReceiver() {
    ...
    private fun getCallkitNotificationManager(context: Context): CallkitNotificationManager? {
        FlutterCallkitIncomingPlugin.ensureManagers(context)
        return FlutterCallkitIncomingPlugin.getInstance()?.getCallkitNotificationManager()
    }

    private fun getCallkitSoundPlayer(context: Context): CallkitSoundPlayerManager? {
        FlutterCallkitIncomingPlugin.ensureManagers(context)
        return FlutterCallkitIncomingPlugin.getInstance()?.getCallkitSoundPlayerManager()
    }
        ...
        when (action) {
            "${context.packageName}.${CallkitConstants.ACTION_CALL_INCOMING}" -> {
                try {
                    ...
                    // Always post the incoming-call notification (CallStyle + ringtone).
