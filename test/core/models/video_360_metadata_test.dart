...
void main() {
  group('Video360Projection.parse', () {
    test('parses equirectangular', () {
      expect(Video360Projection.parse('equirectangular'),
  ...
  group('Video360StereoMode.parse', () {
    test('parses variants', () {
      expect(Video360StereoMode.parse('top_bottom'), Video360StereoMode.topBottom);
      expect(Video360StereoMode.parse('top-bottom'), Video360StereoMode.topBottom);
      expect(Video360StereoMode.parse('side_by_side'), Video360StereoMode.sideBySide);
  ...
  group('sanitize', () {
    test('non-360 collapses to flat', () {
      final m = Video360Metadata.sanitize(
          is360Video: false, projectionType: 'equirectangular', stereoMode: 'mono');
      expect(m.is360Video, isFalse);
      expect(m.use360Player, isFalse);
    });
    test('360 with non-equirectangular downgrades to flat', () {
      final m = Video360Metadata.sanitize(
          is360Video: true, projectionType: 'cubemap', stereoMode: 'mono');
      expect(m.use360Player, isFalse);
    });
    test('valid 360 stays 360', () {
      final m = Video360Metadata.sanitize(
          is360Video: true, projectionType: 'equirectangular', stereoMode: 'mono');
      expect(m.use360Player, isTrue);
    });
  });

  group('forVrPlayback legacy compatibility', () {
    test('legacy isVR post without is360Video is treated as equirectangular', () {
