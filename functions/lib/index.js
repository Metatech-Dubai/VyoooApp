...
exports.generateAgoraTokenOnRequest = (0, firestore_1.onDocumentCreated)({
    ...
    firebase_functions_1.logger.info(`[AgoraToken] received requestId=${requestId} userId=${data.userId} channelName=${data.channelName} uid=${data.uid} role=${data.role}`);
    // ── Validate ───────────────────────────────────────────────────────────────
    if (typeof data.userId !== 'string' ||
        ...
        firebase_functions_1.logger.error(`[AgoraToken] invalid fields requestId=${requestId}`);
        await snap.ref.update({ status: 'error', error: 'Invalid request fields.' });
        return;
    }
    if (!APP_CERTIFICATE) {
        firebase_functions_1.logger.error(`[AgoraToken] missing APP_CERTIFICATE requestId=${requestId}`);
    ...
    try {
        ...
        firebase_functions_1.logger.info(`[AgoraToken] generating token requestId=${requestId} channel="${channelName}" uid=${uid} role=${data.role} expirySeconds=${TOKEN_TTL_SECONDS}`);
        ...
        firebase_functions_1.logger.info(`[AgoraToken] token generated requestId=${requestId} tokenLen=${token.length}`);
        await snap.ref.update({ status: 'done', token, expiresAt: privilegeExpireTime });
    }
    catch (e) {
        firebase_functions_1.logger.error(`[AgoraToken] generation failed requestId=${requestId} error=${String(e)}`);
 ...
 */
exports.applyAcceptedFollowRequest = (0, firestore_1.onDocumentUpdated)({
    ...
    if (!after)
        return;
    const prevStatus = typeof (before === null || before === void 0 ? void 0 : before.status) === 'string' ? before.status : '';
    const nextStatus = typeof after.status === 'string' ? after.status : '';
    if (prevStatus === 'accepted' || nextStatus !== 'accepted')
        return;
    const requesterUid = typeof after.requesterUid === 'string' ? after.requesterUid.trim() : '';
    const targetUid = typeof after.targetUid === 'string' ? after.targetUid.trim() : '';
    if (!requesterUid || !targetUid || requesterUid === targetUid) {
        firebase_functions_1.logger.warn('applyAcceptedFollowRequest: invalid uids', { requesterUid, targetUid });
        return;
    }
    const db = admin.firestore();
    ...
    try {
        ...
        firebase_functions_1.logger.error('applyAcceptedFollowRequest: batch failed', {
            error: String(err),
            requesterUid,
            targetUid,
        });
    ...
    try {
        ...
        firebase_functions_1.logger.warn('applyAcceptedFollowRequest: notification failed', { error: String(err) });
    }
});
...
exports.onCallSessionCreate = (0, firestore_1.onDocumentCreated)({
    ...
    const calleeIds = Array.isArray(data.calleeIds)
        ? data.calleeIds.filter((v) => typeof v === 'string' && v.length > 0)
        : [];
    if (!callerId || calleeIds.length === 0 || !chatId) {
        firebase_functions_1.logger.error('onCallSessionCreate: invalid call data', { callId });
        return;
    }
    ...
    try {
        ...
        if (chatSnap.exists) {
            const chatData = chatSnap.data();
            const pMap = chatData.participantMap;
            const callerInfo = pMap === null || pMap === void 0 ? void 0 : pMap[callerId];
            if (callerInfo) {
        ...
        firebase_functions_1.logger.warn('onCallSessionCreate: could not fetch caller name', { error: String(err) });
    }
    const tokenSnaps = await Promise.all(calleeIds.map((uid) => db.collection('users').doc(uid).collection('push_tokens').get()));
    const tokens = [];
    ...
    if (tokens.length === 0) {
        firebase_functions_1.logger.info('onCallSessionCreate: no push tokens for callees', { callId });
        return;
    }
    ...
    for (let t = 0; t < tokens.length; t += FCM_BATCH) {
        ...
        try {
            ...
            if (response.failureCount > 0) {
                firebase_functions_1.logger.warn('onCallSessionCreate: FCM partial failure', {
                    callId,
                    failureCount: response.failureCount,
                });
            }
            ...
            firebase_functions_1.logger.error('onCallSessionCreate: FCM send error', { callId, error: String(err) });
        }
    }
    firebase_functions_1.logger.info('onCallSessionCreate: push sent', { callId, tokenCount: tokens.length });
});
// ── Calls: onCallSessionUpdate ────────────────────────────────────────────────
exports.onCallSessionUpdate = (0, firestore_1.onDocumentUpdated)({
    document: 'callSessions/{callId}',
    timeoutSeconds: 30,
    memory: '256MiB',
}, async (event) => {
    ...
    const participantIds = Array.isArray(after.participantIds)
        ...
        : [];
    if (!chatId || !callerId) {
        firebase_functions_1.logger.error('onCallSessionUpdate: missing chatId or callerId', { callId });
        return;
    }
    ...
    if (!existingMsg.empty) {
        ...
        firebase_functions_1.logger.info('onCallSessionUpdate: updated existing call message', { callId, newStatus });
        return;
    }
    ...
    firebase_functions_1.logger.info('onCallSessionUpdate: call system message created', { callId, chatId, newStatus });
});
function _callPreviewText(callType, status, durationSeconds) {
...
exports.cleanupStaleRingingCalls = (0, scheduler_1.onSchedule)({
    ...
    if (staleSnap.empty)
        return;
    firebase_functions_1.logger.info('cleanupStaleRingingCalls: found stale calls', { count: staleSnap.size });
    for (const doc of staleSnap.docs) {
        try {
            ...
            firebase_functions_1.logger.error('cleanupStaleRingingCalls: update failed', { callId: doc.id, error: String(err) });
        }
    }
    firebase_functions_1.logger.info('cleanupStaleRingingCalls: done', { processed: staleSnap.size });
});
